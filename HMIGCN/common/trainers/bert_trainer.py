...
class BertTrainer(object):
    ...
    def train_epoch(self, train_dataloader):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            if self.args.is_multilabel:
                loss = F.binary_cross_entropy_with_logits(logits, label_ids.float()) + F.binary_cross_entropy_with_logits(logits1, label_ids.float())
            else:
    ...
    def train(self):
        train_features = convert_examples_to_features(
            self.train_examples, self.args.max_sec_length, self.tokenizer)
        ...
        for epoch in trange(int(self.args.num_train_epochs), desc="Epoch"):
            ...
            if test_f1 > self.best_test_f1:
                ...
                tqdm.write(
                    "Early Stopping. Epoch: {}, unimproved_iters: {},Best Dev F1: {}, Best Test F1: {}".format(
                        epoch, self.unimproved_iters, self.best_test_f1, self.best_test_f1))
